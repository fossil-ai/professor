...
class LabelEncoder:
    ...
    #         ner.py:325 encode_batch  *
    #     label = self._encode_sample(images_shape, gt_boxes[i], cls_ids[i])
    # ner.py:304 _encode_sample  *
    #     matched_gt_idx, positive_mask, ignore_mask = self._match_anchor_boxes(
    # ner.py:276 _match_anchor_boxes  *
    #     iou_matrix = compute_iou(anchor_boxes, gt_boxes)
    # ner.py:70 compute_iou  *
    #     lu = tf.maximum(boxes1_corners[:, None, :2], boxes2_corners[:, :2])

    def encode_batch(self, batch_images, gt_boxes, cls_ids, captions):
        ...
        return {'images': batch_images, 'caption': captions}, labels.stack()
...
train_dataset = tf.data.TFRecordDataset([filename]).map(preprocess_data)
